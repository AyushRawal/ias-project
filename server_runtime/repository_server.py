...
class RepositoryServer:
    ...
    def start(self):
        self.logger.info("Starting Repository Server")
        ...
        try:
            ...
            self.logger.info(f"Repository Server started on port {port}")
            return proc.pid

        except Exception as e:
            self.logger.error(f"Failed to start Repository Server: {e}")
            return -1
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run the Repository Server.")
    parser.add_argument(
