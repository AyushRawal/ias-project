...
class RegistryServer:
    ...
    def _load_config(self):
        ...
        try:
            ...
            self.logger.error(f"Load descriptor failed: {e}")
            return {}
    ...
    def _setup_environment(self):
        ...
        if deps:
            self.logger.info(f"Installing {deps}")
            subprocess.run(["python","-m","venv","env"], cwd=self.app_dir, check=True)
            subprocess.run(["env/bin/python","-m","pip","install","-q"]+deps,
                           cwd=self.app_dir, check=True)
        return True

    def start(self):
        self.logger.info("Starting RegistryServer")
        if not self._setup_environment():
            return -1

        # init_module: runs once
        init = self.config.get("init_module")
        if init:
            pkg, fn = init.split(":")
            cmd = ["env/bin/python","-c",f"import {pkg}; {pkg}.{fn}()"]
            self.logger.info(f"Init: {' '.join(cmd)}")
            subprocess.run(cmd, cwd=self.app_dir, check=True)

        # app_module: serve over Gunicorn
        appmod = self.config.get("app_module","app:app")
        cmd = ["env/bin/gunicorn","-b",f"0.0.0.0:{self.config['port']}", appmod]
